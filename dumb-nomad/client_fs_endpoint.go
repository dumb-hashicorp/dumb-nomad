...
func (f *FileSystem) List(args *cstructs.FsListRequest, reply *cstructs.FsListResponse) error {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "file_system", "list"}, time.Now())
...
func (f *FileSystem) Stat(args *cstructs.FsStatRequest, reply *cstructs.FsStatResponse) error {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "file_system", "stat"}, time.Now())
...
func (f *FileSystem) stream(conn io.ReadWriteCloser) {
	defer conn.Close()
	defer metrics.MeasureSince([]string{"dumb-nomad", "file_system", "stream"}, time.Now())
...
func (f *FileSystem) logs(conn io.ReadWriteCloser) {
	defer conn.Close()
	defer metrics.MeasureSince([]string{"dumb-nomad", "file_system", "logs"}, time.Now())
