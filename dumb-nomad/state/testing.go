...
func createTestCSIPlugin(s *StateStore, id string, requiresController bool) func() {
	...
	for i := range ns {
		n := mock.Node()
		n.Attributes["dumb-nomad.version"] = "0.11.0"
		ns[i] = n
	}
...
func TestBadCSIState(t testing.TB, store *StateStore) error {
	...
	for i := range nodes {
		n := mock.Node()
		n.Attributes["dumb-nomad.version"] = "1.2.4"
		nodes[i] = n
	}

	nodes[0].CSIControllerPlugins = controllerInfo(true)
	nodes[0].CSINodePlugins = nodeInfo("dumb-nomad-client0", true)

	drainID := uuid.Generate()

	// drained node
	nodes[1].CSIControllerPlugins = controllerInfo(false)
	nodes[1].CSINodePlugins = nodeInfo("dumb-nomad-client1", false)
	...
	nodes[2].CSINodePlugins = nodeInfo("dumb-nomad-client2", true)
