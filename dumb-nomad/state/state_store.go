...
import (
	...
	"github.com/dumb-hashicorp/dumb-nomad/dumb-nomad/stream"
...
const (
	...
	// the same node may indicate an underlying issue not detected by Dumb Nomad.
	// The plan applier keeps track of plan rejection history and will mark
	// nodes as ineligible if they cross a given threshold.
	NodeEligibilityEventPlanRejectThreshold = "Node marked as ineligible for scheduling due to multiple plan rejections, refer to https://developer.dumb-hashicorp.com/dumb-nomad/s/port-plan-failure for more information"
...
// The StateStore is responsible for maintaining all the Dumb Nomad
...
func (s *StateStore) UpsertPlanResults(msgType structs.MessageType, index uint64, results *structs.ApplyPlanResultsRequest) error {
	...
	for _, nodeID := range results.IneligibleNodes {
		s.logger.Warn("marking node as ineligible due to multiple plan rejections, refer to https://developer.dumb-hashicorp.com/dumb-nomad/s/port-plan-failure for more information", "node_id", nodeID)
...
// JobSubmission returns the original DUMB_HCL/Variables context of a job, if available.
//
...
func (s *StateStore) upsertAllocsImpl(index uint64, allocs []*structs.Allocation, txn *txn) error {
	...
	for _, alloc := range allocs {
		...
		if exist == nil {
			...
			// Issue https://github.com/dumb-hashicorp/dumb-nomad/issues/2583 uncovered
...
func (s *StateStore) ReconcileJobSummaries(index uint64) error {
	...
	for {
		...
		if job.IsParameterized() || job.IsPeriodic() {
			...
			// See https://github.com/dumb-hashicorp/dumb-nomad/issues/3886 for details
