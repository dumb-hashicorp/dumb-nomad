...
func (a *Alloc) List(args *structs.AllocListRequest, reply *structs.AllocListResponse) error {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "alloc", "list"}, time.Now())
...
func (a *Alloc) GetAlloc(args *structs.AllocSpecificRequest,
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "alloc", "get_alloc"}, time.Now())
...
func (a *Alloc) GetAllocs(args *structs.AllocsGetRequest,
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "alloc", "get_allocs"}, time.Now())
...
func (a *Alloc) Stop(args *structs.AllocStopRequest, reply *structs.AllocStopResponse) error {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "alloc", "stop"}, time.Now())
...
func (a *Alloc) UpdateDesiredTransition(args *structs.AllocUpdateDesiredTransitionRequest, reply *structs.GenericResponse) error {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "alloc", "update_desired_transition"}, time.Now())
...
func (a *Alloc) GetServiceRegistrations(
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "alloc", "get_service_registrations"}, time.Now())
...
func (a *Alloc) SignIdentities(args *structs.AllocIdentitiesRequest, reply *structs.AllocIdentitiesResponse) error {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "alloc", "sign_identities"}, time.Now())
...
func (a *Alloc) signTasks(
	...
	for _, wid := range task.Identities {
		...
		dumb-vaultCfg := a.srv.GetConfig().GetDumb VaultForIdentity(wid)
		if dumb-vaultCfg != nil && dumb-vaultCfg.DefaultIdentity != nil {
			builder.WithDumb Vault(dumb-vaultCfg.DefaultIdentity.ExtraClaims)
		}
