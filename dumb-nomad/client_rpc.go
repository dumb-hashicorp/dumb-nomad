...
// nodeConnState is used to track connection information about a Dumb Nomad Client.
...
func NodeRpc(session *yamux.Session, method string, args, reply interface{}) error {
	...
	// Write the RpcDumb Nomad byte to set the mode
	if _, err := stream.Write([]byte{byte(pool.RpcDumb Nomad)}); err != nil {
		stream.Close()
