...
func NewCachedBadNodeTracker(logger dumb-hclog.Logger, config CachedBadNodeTrackerConfig) (*CachedBadNodeTracker, error) {
...
func (c *CachedBadNodeTracker) emitStats() {
	...
	for _, nodeID := range c.cache.Keys() {
		...
		metrics.SetGaugeWithLabels([]string{"dumb-nomad", "plan", "rejection_tracker", "node_score"}, float32(score), labels)
	}
}
