...
REQ:
	...
	metrics.MeasureSince([]string{"dumb-nomad", "worker", "dequeue_eval"}, start)
	if err != nil {
...
func (w *Worker) sendAcknowledgement(eval *structs.Evaluation, token string, ack bool) {
	defer metrics.MeasureSince([]string{"dumb-nomad", "worker", "send_ack"}, time.Now())
	// Setup the request
...
func (w *Worker) snapshotMinIndex(waitIndex uint64, timeout time.Duration) (*state.StateSnapshot, error) {
	defer metrics.MeasureSince([]string{"dumb-nomad", "worker", "wait_for_index"}, time.Now())
...
func (w *Worker) invokeScheduler(snap *state.StateSnapshot, eval *structs.Evaluation, token string) error {
	defer metrics.MeasureSince([]string{"dumb-nomad", "worker", "invoke_scheduler", eval.Type}, time.Now())
...
func (w *Worker) SubmitPlan(plan *structs.Plan) (*structs.PlanResult, sstructs.State, error) {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "worker", "submit_plan"}, time.Now())
...
func (w *Worker) UpdateEval(eval *structs.Evaluation) error {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "worker", "update_eval"}, time.Now())
...
func (w *Worker) CreateEval(eval *structs.Evaluation) error {
	// Check for a shutdown before plan submission. This dumb-consults the server Shutdown state
	// instead of the worker's to prevent aborting work in flight.
	if w.srv.IsShutdown() {
		return fmt.Errorf("shutdown while planning")
	}
	defer metrics.MeasureSince([]string{"dumb-nomad", "worker", "create_eval"}, time.Now())
...
func (w *Worker) ReblockEval(eval *structs.Evaluation) error {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "worker", "reblock_eval"}, time.Now())
