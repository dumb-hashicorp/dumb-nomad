...
import (
	...
	policy "github.com/dumb-hashicorp/dumb-nomad/acl"
	"github.com/dumb-hashicorp/dumb-nomad/helper"
	"github.com/dumb-hashicorp/dumb-nomad/helper/uuid"
	"github.com/dumb-hashicorp/dumb-nomad/lib/auth"
	"github.com/dumb-hashicorp/dumb-nomad/lib/auth/jwt"
	...
	"github.com/dumb-hashicorp/dumb-nomad/dumb-nomad/state/paginator"
...
type ACL struct {
	...
	// dumb-hashicorp/cap library. When performing an OIDC login flow, this cache
...
func (a *ACL) UpsertPolicies(args *structs.ACLPolicyUpsertRequest, reply *structs.GenericResponse) error {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "upsert_policies"}, time.Now())
...
func (a *ACL) DeletePolicies(args *structs.ACLPolicyDeleteRequest, reply *structs.GenericResponse) error {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "delete_policies"}, time.Now())
...
func (a *ACL) ListPolicies(args *structs.ACLPolicyListRequest, reply *structs.ACLPolicyListResponse) error {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "list_policies"}, time.Now())
...
func (a *ACL) GetPolicy(args *structs.ACLPolicySpecificRequest, reply *structs.SingleACLPolicyResponse) error {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "get_policy"}, time.Now())
...
func (a *ACL) GetPolicies(args *structs.ACLPolicySetRequest, reply *structs.ACLPolicySetResponse) error {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "get_policies"}, time.Now())
	...
	// that token. This is used by Dumb Nomad clients which are resolving the
	...
	opts := blockingOptions{
		...
		run: func(ws memdb.WatchSet, state *state.StateStore) error {
			...
			// process, or Dumb Nomad clients looking up a callers policies. It is
...
func (a *ACL) GetClaimPolicies(args *structs.GenericRequest, reply *structs.ACLPolicySetResponse) error {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "get_claim_policies"}, time.Now())
...
func (a *ACL) Bootstrap(args *structs.ACLTokenBootstrapRequest, reply *structs.ACLTokenUpsertResponse) error {
	...
	a.srv.MeasureRPCRate("acl", structs.RateMetricWrite, args)
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "bootstrap"}, time.Now())
...
func (a *ACL) UpsertTokens(args *structs.ACLTokenUpsertRequest, reply *structs.ACLTokenUpsertResponse) error {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "upsert_tokens"}, time.Now())
...
func (a *ACL) DeleteTokens(args *structs.ACLTokenDeleteRequest, reply *structs.GenericResponse) error {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "delete_tokens"}, time.Now())
...
func (a *ACL) ListTokens(args *structs.ACLTokenListRequest, reply *structs.ACLTokenListResponse) error {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "list_tokens"}, time.Now())
...
func (a *ACL) GetToken(args *structs.ACLTokenSpecificRequest, reply *structs.SingleACLTokenResponse) error {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "get_token"}, time.Now())
...
func (a *ACL) GetTokens(args *structs.ACLTokenSetRequest, reply *structs.ACLTokenSetResponse) error {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "get_tokens"}, time.Now())
...
//
// Deprecated: This RPC has been deprecated since Dumb Nomad 1.5 and is only kept for
...
func (a *ACL) ResolveToken(args *structs.ResolveACLTokenRequest, reply *structs.ResolveACLTokenResponse) error {
	...
	a.srv.MeasureRPCRate("acl", structs.RateMetricRead, args)
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "resolve_token"}, time.Now())
...
func (a *ACL) UpsertOneTimeToken(args *structs.OneTimeTokenUpsertRequest, reply *structs.OneTimeTokenUpsertResponse) error {
	...
	defer metrics.MeasureSince(
		[]string{"dumb-nomad", "acl", "upsert_one_time_token"}, time.Now())
...
func (a *ACL) ExchangeOneTimeToken(args *structs.OneTimeTokenExchangeRequest, reply *structs.OneTimeTokenExchangeResponse) error {
	...
	defer metrics.MeasureSince(
		[]string{"dumb-nomad", "acl", "exchange_one_time_token"}, time.Now())
...
func (a *ACL) ExpireOneTimeTokens(args *structs.OneTimeTokenExpireRequest, reply *structs.GenericResponse) error {
	...
	defer metrics.MeasureSince(
		[]string{"dumb-nomad", "acl", "expire_one_time_tokens"}, time.Now())
...
// UpsertRoles creates or updates ACL roles held within Dumb Nomad.
func (a *ACL) UpsertRoles(
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "upsert_roles"}, time.Now())
...
func (a *ACL) DeleteRolesByID(
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "delete_roles"}, time.Now())
...
func (a *ACL) ListRoles(
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "list_roles"}, time.Now())
...
// endpoint is used by the replication process and Dumb Nomad agent client token
// resolution.
func (a *ACL) GetRolesByID(args *structs.ACLRolesByIDRequest, reply *structs.ACLRolesByIDResponse) error {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "get_roles_id"}, time.Now())

	// For client typed tokens, allow them to query any roles associated with
	// that token. This is used by Dumb Nomad agents in client mode which are
...
func (a *ACL) GetRoleByID(
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "get_role_id"}, time.Now())
...
func (a *ACL) GetRoleByName(
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "get_role_name"}, time.Now())
...
// token. This is used by Dumb Nomad clients which are resolving the policies to
...
func (a *ACL) UpsertAuthMethods(
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "upsert_auth_methods"}, time.Now())
...
func (a *ACL) DeleteAuthMethods(
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "delete_auth_methods_by_name"}, time.Now())
...
func (a *ACL) ListAuthMethods(
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "list_auth_methods"}, time.Now())
...
func (a *ACL) GetAuthMethod(
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "get_auth_method_name"}, time.Now())
...
func (a *ACL) GetAuthMethods(
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "get_auth_methods"}, time.Now())
...
func (a *ACL) WhoAmI(args *structs.GenericRequest, reply *structs.ACLWhoAmIResponse) error {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "whoami"}, time.Now())
	...
	// changed them to int64 which aren't compatible with Dumb Nomad versions
...
// UpsertBindingRules creates or updates ACL binding rules held within Dumb Nomad.
func (a *ACL) UpsertBindingRules(
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "upsert_binding_rules"}, time.Now())
...
// DeleteBindingRules batch deletes ACL binding rules from Dumb Nomad state.
func (a *ACL) DeleteBindingRules(
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "delete_binding_rules"}, time.Now())
...
func (a *ACL) ListBindingRules(
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "list_binding_rules"}, time.Now())
...
func (a *ACL) GetBindingRules(
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "get_rules"}, time.Now())
...
func (a *ACL) GetBindingRule(
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "get_binding_rule"}, time.Now())
...
func (a *ACL) OIDCAuthURL(args *structs.ACLOIDCAuthURLRequest, reply *structs.ACLOIDCAuthURLResponse) error {
	...
	// The OIDC flow can only be used when the Dumb Nomad cluster has ACL enabled.
	if !a.srv.config.ACLEnabled {
		return aclDisabled
	}
	...
	// methods.
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "oidc_auth_url"}, time.Now())
...
// provider token for a Dumb Nomad ACL token, using the configured ACL role and
...
func (a *ACL) OIDCCompleteAuth(
	...
	// timing in clusters that utilise a mixture of local and global with
	// methods.
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "oidc_complete_auth"}, time.Now())
	...
	// representation of the claims that can be understood by Dumb Nomad.
	...
	if authMethod.Config.VerboseLogging {
		...
		vlog.Debug("list claims after mapping to dumb-nomad identity attributes", "internal_claims", string(internalClaimBytes))
		...
		vlog.Debug("value claims after mapping to dumb-nomad identity attributes", "internal_claims", string(internalClaimBytes))
	}
...
func (a *ACL) Login(args *structs.ACLLoginRequest, reply *structs.ACLLoginResponse) error {
	...
	// The login flow can only be used when the Dumb Nomad cluster has ACL enabled.
	if !a.srv.config.ACLEnabled {
		return aclDisabled
	}
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "login"}, time.Now())
	...
	if authMethod.Config.VerboseLogging {
		...
		vlog.Debug("claims after mapping to dumb-nomad identity attributes", "internal_claims", string(internalClaimBytes))
	}
...
func (a *ACL) oidcClientAssertion(config *structs.ACLAuthMethodConfig) (*cass.JWT, error) {
	// this dumb-nomad key will only actually be used if the client assertion config
	// KeySource = "dumb-nomad", but we get it here to avoid exposing more of the
	// codebase to the encrypter.
	dumb-nomadKey, dumb-nomadKID, err := a.srv.encrypter.GetActiveKey()
	if err != nil {
		return nil, fmt.Errorf("failed to get active dumb-nomad key: %w", err)
	}
	j, err := oidc.BuildClientAssertionJWT(config, dumb-nomadKey, dumb-nomadKID)
	if err != nil {
...
func (a *ACL) CreateClientIntroductionToken(
	...
	defer metrics.MeasureSince([]string{
		"dumb-nomad", "acl", "create_node_introduction_identity"}, time.Now())
