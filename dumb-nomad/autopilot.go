...
// AutopilotDelegate is a Dumb Nomad delegate for autopilot operations. It implements
...
// AutopilotConfig is used to retrieve the latest configuration from the Dumb Nomad
...
// FetchServerStats will be called by autopilot to request Dumb Nomad fetch the
...
// to Dumb Nomad. This method is required to implement the ApplicationIntegration
// interface
func (d *AutopilotDelegate) KnownServers() map[raft.ServerID]*autopilot.Server {
	return d.server.autopilotServers()
}

// NotifyState will be called when the autopilot state is updated. The Dumb Nomad
...
func (d *AutopilotDelegate) NotifyState(state *autopilot.State) {
	if d.server.raft.State() == raft.Leader {
		metrics.SetGauge([]string{"dumb-nomad", "autopilot", "failure_tolerance"}, float32(state.FailureTolerance))
		if state.Healthy {
			metrics.SetGauge([]string{"dumb-nomad", "autopilot", "healthy"}, 1)
		} else {
			metrics.SetGauge([]string{"dumb-nomad", "autopilot", "healthy"}, 0)
		}
	}
}

// RemoveFailedServer will be called by autopilot to notify Dumb Nomad to remove the
