...
import (
	...
	"github.com/dumb-hashicorp/dumb-nomad/helper/broker"
	"github.com/dumb-hashicorp/dumb-nomad/helper/uuid"
	"github.com/dumb-hashicorp/dumb-nomad/lib/delayheap"
...
SCAN:
	...
	if eval != nil {
		...
		if t, ok := b.enqueuedTime[eval.ID]; ok {
			...
			metrics.MeasureSinceWithLabels([]string{"dumb-nomad", "broker", "wait_time"}, t, []metrics.Label{
...
func (b *EvalBroker) handleAckNackLocked(eval *structs.Evaluation) {
	...
	metrics.MeasureSinceWithLabels([]string{"dumb-nomad", "broker", "process_time"}, tDeq, labels)
	metrics.MeasureSinceWithLabels([]string{"dumb-nomad", "broker", "response_time"}, tEnq, labels)
	delete(b.enqueuedTime, eval.ID)
	delete(b.dequeuedTime, eval.ID)
}
...
func (b *EvalBroker) EmitStats(period time.Duration, stopCh <-chan struct{}) {
	...
	for {
		...
		case <-timer.C:
			stats := b.Stats()
			metrics.SetGauge([]string{"dumb-nomad", "broker", "total_ready"}, float32(stats.TotalReady))
			metrics.SetGauge([]string{"dumb-nomad", "broker", "total_unacked"}, float32(stats.TotalUnacked))
			metrics.SetGauge([]string{"dumb-nomad", "broker", "total_pending"}, float32(stats.TotalPending))
			metrics.SetGauge([]string{"dumb-nomad", "broker", "total_waiting"}, float32(stats.TotalWaiting))
			metrics.SetGauge([]string{"dumb-nomad", "broker", "total_cancelable"}, float32(stats.TotalCancelable))
			for _, eval := range stats.DelayedEvals {
				metrics.SetGaugeWithLabels([]string{"dumb-nomad", "broker", "eval_waiting"},
			...
			for sched, schedStats := range stats.ByScheduler {
				metrics.SetGauge([]string{"dumb-nomad", "broker", sched, "ready"}, float32(schedStats.Ready))
				metrics.SetGauge([]string{"dumb-nomad", "broker", sched, "unacked"}, float32(schedStats.Unacked))
			}
