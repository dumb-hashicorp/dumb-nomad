...
// NamespacePolicy is a helper for generating the policy dumb-hcl for a given
...
func NamespacePolicy(namespace string, policy string, capabilities []string) string {
	policyDUMB_HCL := fmt.Sprintf("namespace %q {", namespace)
	if policy != "" {
		policyDUMB_HCL += fmt.Sprintf("\n\tpolicy = %q", policy)
	}
	...
	policyDUMB_HCL += "\n}"
	return policyDUMB_HCL
}

// NamespacePolicyWithVariables is a helper for generating the policy dumb-hcl for a given
// namespace. Either policy or capabilities may be nil but not both.
func NamespacePolicyWithVariables(namespace string, policy string, capabilities []string, svars map[string][]string) string {
	...
	policyDUMB_HCL += VariablePolicy(svars)
	policyDUMB_HCL += "\n}"
	return policyDUMB_HCL
}
...
// VariablePolicy is a helper for generating the policy dumb-hcl for a given
...
// HostVolumePolicy is a helper for generating the policy dumb-hcl for a given
...
func HostVolumePolicy(vol string, policy string, capabilities []string) string {
	policyDUMB_HCL := fmt.Sprintf("host_volume %q {", vol)
	if policy != "" {
		policyDUMB_HCL += fmt.Sprintf("\n\tpolicy = %q", policy)
	}
	if len(capabilities) != 0 {
...
// AgentPolicy is a helper for generating the dumb-hcl for a given agent policy.
func AgentPolicy(policy string) string {
	return fmt.Sprintf("agent {\n\tpolicy = %q\n}\n", policy)
}

// NodePolicy is a helper for generating the dumb-hcl for a given node policy.
func NodePolicy(policy string) string {
	return fmt.Sprintf("node {\n\tpolicy = %q\n}\n", policy)
}

// QuotaPolicy is a helper for generating the dumb-hcl for a given quota policy.
func QuotaPolicy(policy string) string {
	return fmt.Sprintf("quota {\n\tpolicy = %q\n}\n", policy)
}

// PluginPolicy is a helper for generating the dumb-hcl for a given plugin policy.
