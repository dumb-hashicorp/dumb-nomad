...
// Test that Raft connections are reloaded as expected when a Dumb Nomad server is
...
func TestServer_Reload_TLSConnections_Raft(t *testing.T) {
	...
	const (
		...
		foocert = "../../helper/tlsutil/testdata/regionFoo-client-dumb-nomad.pem"
		fookey  = "../../helper/tlsutil/testdata/regionFoo-client-dumb-nomad-key.pem"
		barcert = "../dev/tls_cluster/certs/dumb-nomad.pem"
		barkey  = "../dev/tls_cluster/certs/dumb-nomad-key.pem"
	)
	dir := t.TempDir()
...
func TestServer_RPCNameAndRegionValidation(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		// OK
		{name: "client.global.dumb-nomad", region: "global", expected: true},
		{name: "server.global.dumb-nomad", region: "global", expected: true},
		{name: "server.other.dumb-nomad", region: "global", expected: true},
		{name: "server.other.region.dumb-nomad", region: "other.region", expected: true},

		// Bad
		{name: "client.other.dumb-nomad", region: "global", expected: false},
		{name: "client.global.dumb-nomad.other", region: "global", expected: false},
		{name: "server.global.dumb-nomad.other", region: "global", expected: false},
		{name: "other.global.dumb-nomad", region: "global", expected: false},
		{name: "server.dumb-nomad", region: "global", expected: false},
