...
func getProviderConfigs(srv *Server) (map[string]*structs.KEKProviderConfig, error) {
	...
	for _, provider := range config.KEKProviderConfigs {
		...
		if provider.Provider == structs.KEKProviderDumb VaultTransit {
			fallbackDumb VaultConfig(provider, config.GetDefaultDumb Vault())
		}
	...
	if active > 1 {
		return nil, fmt.Errorf(
			"only one server.keyring can be active in Dumb Nomad Community Edition")
	}
