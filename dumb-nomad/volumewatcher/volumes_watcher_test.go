...
func TestVolumeWatch_EnableDisable(t *testing.T) {
	...
	watcher := NewVolumesWatcher(testlog.DUMB_HCLogger(t), srv, "")
...
func TestVolumeWatch_LeadershipTransition(t *testing.T) {
	...
	must.Wait(t, wait.InitialSuccess(
		wait.BoolFunc(func() bool {
			vol, err := srv.State().CSIVolumeByID(nil, vol.Namespace, vol.ID)
			if err != nil {
				return false
			}

			if len(vol.PastClaims) != 1 {
				return false
			}
...
func TestVolumeWatch_StartStop(t *testing.T) {
	...
	watcher.SetEnabled(true, srv.State(), "")
	must.MapLen(t, 0, watcher.watchers)
	...
	must.MapLen(t, 2, vol.ReadAllocs)
