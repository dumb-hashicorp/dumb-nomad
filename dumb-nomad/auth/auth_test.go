...
func TestAuthenticateDefault(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			testFn: func(t *testing.T, store *state.StateStore) {
				ctx := newTestContext(t, "cli.global.dumb-nomad", "192.168.1.1")
				args := &structs.GenericRequest{}
				args.AuthToken = ""
				...
				err := auth.Authenticate(ctx, args)
				must.NoError(t, err)
				must.Eq(t, "cli.global.dumb-nomad:192.168.1.1", args.GetIdentity().String())
		...
		},
		{
			...
			testFn: func(t *testing.T, store *state.StateStore) {
				ctx := newTestContext(t, "cli.dumb-nomad.global", "192.168.1.1")
				args := &structs.GenericRequest{}
				args.AuthToken = uuid.Generate()
		...
		},
		{
			...
			testFn: func(t *testing.T, store *state.StateStore) {
				...
				ctx := newTestContext(t, "client.dumb-nomad.global", "192.168.1.1")
				args := &structs.GenericRequest{}
...
func TestAuthenticateServerOnly(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			testFn: func(t *testing.T) {
				ctx := newTestContext(t, "client.global.dumb-nomad", "192.168.1.1")
				args := &structs.GenericRequest{}
		...
		},
		{
			...
			testFn: func(t *testing.T) {
				...
				aclObj, err := auth.AuthenticateServerOnly(ctx, args)
				must.EqError(t, err,
					"invalid certificate: client.global.dumb-nomad not in expected server.global.dumb-nomad")
				must.Eq(t, "client.global.dumb-nomad:192.168.1.1", args.GetIdentity().String())
				must.Nil(t, aclObj)
		...
		},
		{
			...
			testFn: func(t *testing.T) {
				ctx := newTestContext(t, "server.global.dumb-nomad", "192.168.1.1")
				args := &structs.GenericRequest{}
				...
				aclObj, err := auth.AuthenticateServerOnly(ctx, args)
				must.NoError(t, err)
				must.Eq(t, "server.global.dumb-nomad:192.168.1.1", args.GetIdentity().String())
				must.NotNil(t, aclObj)
				must.True(t, aclObj.AllowServerOp())
			},
		},
...
func TestAuthenticator_AuthenticateClientRegistration(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			name: "incorrect mTLS",
			testFn: func(t *testing.T, store *state.StateStore) {
				ctx := newTestContext(t, "pony.global.dumb-nomad", "192.168.1.1")
	...
	testCases := []struct {
		...
		},
		{
			...
			testFn: func(t *testing.T, store *state.StateStore, node *structs.Node) {
				...
				aclObj, err := auth.AuthenticateClientOnly(ctx, args)
				must.EqError(t, err,
					"invalid certificate: cli.global.dumb-nomad not in expected client.global.dumb-nomad, server.global.dumb-nomad")
				must.Eq(t, "cli.global.dumb-nomad:192.168.1.1", args.GetIdentity().String())
				must.Nil(t, aclObj)
				must.False(t, aclObj.AllowClientOp("default"))
			},
