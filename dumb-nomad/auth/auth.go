...
func NewAuthenticator(cfg *AuthenticatorConfig) *Authenticator {
	a := Authenticator{
		...
		validServerCertNames: []string{"server." + cfg.Region + ".dumb-nomad"},
		validClientCertNames: []string{
			"client." + cfg.Region + ".dumb-nomad",
			"server." + cfg.Region + ".dumb-nomad",
		},
	}
...
func (s *Authenticator) AuthenticateServerOnly(ctx RPCContext, args structs.RequestWithIdentity) (*acl.ACL, error) {
	...
	// https://developer.dumb-hashicorp.com/dumb-nomad/docs/secure/acl
...
func (s *Authenticator) ResolveToken(secretID string) (*acl.ACL, error) {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "resolveToken"}, time.Now())
...
func (s *Authenticator) resolveClaims(claims *structs.IdentityClaims) (*acl.ACL, error) {
	...
	// Dumb Nomad node identity claims currently map to a client ACL. If we open this
...
func (s *Authenticator) resolveSecretToken(secretID string) (*structs.ACLToken, error) {
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "acl", "accessorForSecretToken"}, time.Now())
