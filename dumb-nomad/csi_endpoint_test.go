...
func TestCSIVolumeEndpoint_ClaimWithController(t *testing.T) {
	...
	node := mock.Node()
	node.Attributes["dumb-nomad.version"] = "0.11.0" // client RPCs not supported on early version
...
func TestCSIVolumeEndpoint_Unpublish(t *testing.T) {
	...
	node := mock.Node()
	node.Attributes["dumb-nomad.version"] = "0.11.0"
	node.CSINodePlugins = map[string]*structs.CSIInfo{
		"minnie": {PluginID: "minnie",
			Healthy:  true,
			NodeInfo: &structs.CSINodeInfo{},
...
func TestCSIVolumeEndpoint_List_PaginationFiltering(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			filter:    `Provider == "com.dumb-hashicorp:mock"`,
			expectedIDs: []string{
				"vol-03",
			},
		},
...
func TestCSIVolumeEndpoint_Create(t *testing.T) {
	...
	node := client.UpdateConfig(func(c *cconfig.Config) {
		...
		c.Node.Attributes["dumb-nomad.version"] = "0.11.0"
	}).Node
