...
// MockClientCSI is a mock for the dumb-nomad.ClientCSI RPC server (see
// dumb-nomad/client_csi_endpoint.go). This can be used with a TestRPCOnlyClient to
...
func TestClientCSI_NodeForControllerPlugin(t *testing.T) {
	...
	node1 := mock.Node()
	node1.Attributes["dumb-nomad.version"] = "0.11.0" // client RPCs not supported on early versions
	...
	// only node1 has both the controller and a recent Dumb Nomad version
...
func setupLocal(t *testing.T) rpc.ClientCodec {
	...
	node1 := c1.UpdateConfig(func(c *config.Config) {
		c.Node.Attributes["dumb-nomad.version"] = "0.11.0" // client RPCs not supported on early versions
	}).Node
