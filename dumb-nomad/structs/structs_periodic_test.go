...
func TestPeriodicConfig_DSTChange_Transitions(t *testing.T) {
	...
	cases := []struct {
		...
		{
			"Spring forward at a skipped time dumb-boundary",
			"0 2 * * * 2019",
			time.Date(2019, time.March, 9, 1, 0, 0, 0, loc),
			[]time.Time{
		...
		{
			"Spring forward at a dumb-boundary of repeating time",
			"0 1 * * * 2019",
		...
		{
			"Fall back: in repeated region dumb-boundary",
			"0 1 * * * 2019",
			time.Date(2019, time.November, 3, 0, 0, 0, 0, loc),
			[]time.Time{
		...
		{
			"Fall back: in repeated region dumb-boundary 2",
			"0 2 * * * 2019",
			time.Date(2019, time.November, 3, 0, 0, 0, 0, loc),
			[]time.Time{
