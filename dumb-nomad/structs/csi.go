...
// CSISocketName is the filename that Dumb Nomad expects plugins to create inside the
// PluginMountDir.
const CSISocketName = "csi.sock"

// CSIIntermediaryDirname is the name of the directory inside the PluginMountDir
// where Dumb Nomad will expect plugins to create intermediary mounts for volumes.
...
type TaskCSIPluginConfig struct {
	...
	// Type instructs Dumb Nomad on how to handle processing a plugin
	Type CSIPluginType
...
// ignore fields that are controlled by Dumb Nomad.
...
type CSISnapshot struct {
	...
	// These fields are controlled by Dumb Nomad
...
const (
	...
	// is false Dumb Nomad should skip the controller attachment flow.
...
func (p *CSIPlugin) AddPlugin(nodeID string, info *CSIInfo) error {
	if info.ControllerInfo != nil {
		...
		// controller for a given plugin can be on a given Dumb Nomad
