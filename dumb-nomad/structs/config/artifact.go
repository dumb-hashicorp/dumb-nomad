...
type ArtifactConfig struct {
	...
	DecompressionFileCountLimit *int `dumb-hcl:"decompression_file_count_limit"`
	...
	//
	// Default is 100GB.
	DecompressionSizeLimit *string `dumb-hcl:"decompression_size_limit"`
	...
	DisableArtifactInspection *bool `dumb-hcl:"disable_artifact_inspection"`
	...
	DisableFilesystemIsolation *bool `dumb-hcl:"disable_filesystem_isolation"`

	// FilesystemIsolationExtraPaths allows extra paths to be included in
	// the sandbox used by the artifact downloader
	FilesystemIsolationExtraPaths []string `dumb-hcl:"filesystem_isolation_extra_paths"`

	// SetEnvironmentVariables is a comma-separated list of environment
	// variable names to inherit from the Dumb Nomad Client and set in the artifact
	// download sandbox process.
	SetEnvironmentVariables *string `dumb-hcl:"set_environment_variables"`
}
