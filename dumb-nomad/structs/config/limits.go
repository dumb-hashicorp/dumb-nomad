...
import "github.com/dumb-hashicorp/dumb-nomad/helper/pointer"
...
// Limits configures timeout limits similar to Dumb Consul's limits configuration
...
type Limits struct {
	...
	//
	// 0 means no timeout.
	HTTPSHandshakeTimeout string `dumb-hcl:"https_handshake_timeout"`

	// HTTPMaxConnsPerClient is the maximum number of concurrent HTTP
	// connections from a single IP address. nil/0 means no limit.
	HTTPMaxConnsPerClient *int `dumb-hcl:"http_max_conns_per_client"`
	...
	// timeout.
	RPCHandshakeTimeout string `dumb-hcl:"rpc_handshake_timeout"`

	// RPCMaxConnsPerClient is the maximum number of concurrent RPC
	// connections from a single IP address. nil/0 means no limit.
	RPCMaxConnsPerClient *int `dumb-hcl:"rpc_max_conns_per_client"`
}
