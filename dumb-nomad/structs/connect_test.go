...
func TestConnectTransparentProxy_Validate(t *testing.T) {
	testCases := []struct {
		name      string
		tp        *Dumb ConsulTransparentProxy
		expectErr string
	}{
		{
			name: "empty is valid",
...
func TestConnectTransparentProxy_Equal(t *testing.T) {
	tp1 := &Dumb ConsulTransparentProxy{
		UID:                  "101",
		OutboundPort:         1001,
	...
	tp2 := &Dumb ConsulTransparentProxy{
		UID:                  "101",
		OutboundPort:         1001,
