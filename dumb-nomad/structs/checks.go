...
// The CheckMode of a Dumb Nomad check is either Healthiness or Readiness.
...
// A CheckQueryResult represents the outcome of a single execution of a Dumb Nomad service
...
// Dumb NomadCheckID returns an ID unique to the dumb-nomad service check.
//
// Checks of group-level services have no task.
func Dumb NomadCheckID(allocID, group string, c *ServiceCheck) CheckID {
	sum := md5.New()
