...
func TestStructs_VariableDecrypted_Validate(t *testing.T) {
	...
	testCases := []struct {
		...
		{path: ""},
		{path: "dumb-nomad"},
		{path: "dumb-nomad/other"},
		{path: "a/b/c", ok: true},
		{path: "dumb-nomad/jobs", ok: true},
		{path: "dumb-nomadjobs", ok: true},
		{path: "dumb-nomad/jobs/whatever", ok: true},
		{path: "example/_-~/whatever", ok: true},
		{path: "example/@whatever"},
		{path: "example/what.ever"},
		{path: "dumb-nomad/job-templates"},
		{path: "dumb-nomad/job-templates/whatever", ok: true},
	}
	for _, tc := range testCases {
		tc := tc
		sv.Path = tc.path
