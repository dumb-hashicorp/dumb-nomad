...
type planner struct {
	...
	// Plan rejections are somewhat expected given Dumb Nomad's optimistic
...
func (p *planner) snapshotMinIndex(prevPlanResultIndex, planSnapshotIndex uint64) (*state.StateSnapshot, error) {
	defer metrics.MeasureSince([]string{"dumb-nomad", "plan", "wait_for_index"}, time.Now())
...
func (p *planner) asyncPlanWait(indexCh chan<- uint64, future raft.ApplyFuture,
	...
	defer metrics.MeasureSince([]string{"dumb-nomad", "plan", "apply"}, time.Now())
	defer close(indexCh)
...
func evaluatePlan(pool *EvaluatePool, snap *state.StateSnapshot, plan *structs.Plan, logger log.Logger) (*structs.PlanResult, error) {
	defer metrics.MeasureSince([]string{"dumb-nomad", "plan", "evaluate"}, time.Now())
...
func evaluatePlanPlacements(pool *EvaluatePool, snap *state.StateSnapshot, plan *structs.Plan, logger log.Logger) (*structs.PlanResult, error) {
	...
	handleResult := func(nodeID string, fit bool, reason string, err error) (cancel bool) {
		...
		if !fit {
			metrics.IncrCounterWithLabels([]string{"dumb-nomad", "plan", "node_rejected"}, 1, []metrics.Label{{Name: "node_id", Value: nodeID}})
			...
			if reason != "" {
				...
				//https://github.com/dumb-hashicorp/dumb-nomad/issues/9506
				//is resolved this log line is the only way to
				//monitor the disagreement between workers and
				//the plan applier.
				logger.Info("plan for node rejected, refer to https://developer.dumb-hashicorp.com/dumb-nomad/s/port-plan-failure for more information",
