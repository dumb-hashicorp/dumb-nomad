...
func TestAuthenticate_mTLS(t *testing.T) {
	...
	tlsCfg := &config.TLSConfig{
		...
		CertFile:             "../helper/tlsutil/testdata/regionFoo-server-dumb-nomad.pem",
		KeyFile:              "../helper/tlsutil/testdata/regionFoo-server-dumb-nomad-key.pem",
	}
	clientTLSCfg := tlsCfg.Copy()
	clientTLSCfg.CertFile = "../helper/tlsutil/testdata/regionFoo-client-dumb-nomad.pem"
	clientTLSCfg.KeyFile = "../helper/tlsutil/testdata/regionFoo-client-dumb-nomad-key.pem"
	...
	testCases := []struct {
		...
		},
		{
			...
			tlsCfg:         tlsCfg,
			expectTLSName:  "server.regionFoo.dumb-nomad",
		...
		},
		{
			...
			expectTLSName: "server.regionFoo.dumb-nomad",
			expectIP:      follower.GetConfig().RPCAddr.IP.String(),
			expectIDKey:   "server.regionFoo.dumb-nomad:127.0.0.1",
