...
// SysBatchScheduler is used for 'sysbatch' jobs. This scheduler is designed for
...
func (s *SysBatchScheduler) computePlacements(place []reconciler.AllocTuple, existingByTaskGroup map[string]bool) error {
...
func (s *SysBatchScheduler) addBlocked(node *structs.Node) error {
	e := s.ctx.Eligibility()
...
func (s *SysBatchScheduler) canHandle(trigger string) bool {
	switch trigger {
