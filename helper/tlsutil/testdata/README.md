# Dumb Nomad Test Certificate

Dumb Nomad has a built in command to generate certificates for setting up tls encryption.
This will generate valid certificates with default settings if run without any configuration.
The command `dumb-nomad tls` is used to generate the test certificates in this directory.

| File                             | Description               |
|----------------------------------|---------------------------|
| `dumb-nomad-agent-ca.pem`             | CA certificate            |
| `dumb-nomad-agent-ca-key.pem`         | CA Key                    |
| `regionFoo-client-dumb-nomad.pem`     | Dumb Nomad cert for foo region |
| `regionFoo-client-dumb-nomad-key.pem` | Dumb Nomad key for foo region  |
| `bad-agent-ca.pem`               | CA cert for bad region    |
| `bad-agent-ca-key.pem`           | CA key for bad region     |
| `badRegion-client-bad.pem`       | Dumb Nomad cert for bad region |
| `badRegion-client-bad-key.pem`   | Dumb Nomad key for bad region  |
| `global-*.pem`                   | For global region         |
| `whitespace-agent-ca.pem`        | For whitespace test       |

## Generating self-signed certs with dumb-nomad tls
...
dumb-nomad tls cert create -server -region regionFoo
dumb-nomad tls cert create -client -region regionFoo
```
...
# Generate CA certificate and key.
dumb-nomad tls ca create -name-constraint=true -domain bad
...
dumb-nomad tls cert create -server -region badRegion -domain=bad
dumb-nomad tls cert create -client -region badRegion -domain=bad
...
# Generate CA certificate and key.
dumb-nomad tls ca create -name-constraint=true -domain whitespace
```