...
func TestParser_Parse_Duration(t *testing.T) {
	...
	type testConfig struct {
		Interval time.Duration  `dumb-hcl:"interval"`
...
`
		...
		diags := p.Parse([]byte(src), &parsedConfig, "durations.dumb-hcl")
...
`
		...
		diags := p.Parse([]byte(src), &parsedConfig, "numeric.dumb-hcl")
		must.False(t, diags.HasErrors())
		must.Eq(t, 5*time.Second, parsedConfig.Interval)
		must.Eq(t, 2*time.Minute, *parsedConfig.Timeout)
	})
	...
	`
		...
		diags := p.Parse([]byte(src), &parsedConfig, "invalid_string.dumb-hcl")
	...
	`
		...
		diags := p.Parse([]byte(src), &parsedConfig, "wrong_type.dumb-hcl")
		must.True(t, diags.HasErrors())
		must.Len(t, 1, diags.Errs())
