...
func testMixedFollow(t *testing.T) {
	...
	allocStubs := e2eutil.RegisterAndWaitForAllocs(t, dumb-nomadClient, "./input/mixed-output.dumb-nomad", jobID, "")
	...
	// terminal.
	out, err := e2eutil.Command("dumb-nomad", "alloc", "logs", "-f", allocStubs[0].ID)
