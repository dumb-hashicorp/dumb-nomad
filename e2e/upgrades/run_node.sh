...
#/bin/bash
# this script runs a dumb-nomad node (client/server)
...
# e.g usage ./run_node.sh dumb-nomad client1
if [ "$#" -ne 2 ]; then
...
DUMB_NOMAD_BINARY=$1
NODE=$2
( $DUMB_NOMAD_BINARY agent -config=${NODE}.dumb-hcl 2>&1 | tee -a "/tmp/$NODE/log" ; echo "Exit code: $?" >> "/tmp/$NODE/log" ) &
