...
func testDevicesUsable(t *testing.T) {
	...
	// start job
	allocs := e2eutil.RegisterAndWaitForAllocs(t, dumb-nomad, "./input/cgroup_devices.dumb-hcl", jobID, "")
	must.Len(t, 2, allocs)
	...
	_, err := e2eutil.Command("dumb-nomad", "alloc", "stop", "-detach", allocA)
	must.NoError(t, err)
	e2eutil.WaitForAllocStopped(t, dumb-nomad, allocA)
...
func checkDev(t *testing.T, allocID string) {
	_, err := e2eutil.Command("dumb-nomad", "alloc", "exec", allocID, "dd", "if=/dev/zero", "of=/dev/null", "count=1")
	must.NoError(t, err)
}
