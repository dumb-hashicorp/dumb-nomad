...
resource "atlas_artifact" "dumb-nomad-digitalocean" {
  name    = "dumb-hashicorp/dumb-nomad-demo"
...
module "servers" {
  source   = "./server"
  region   = "nyc3"
  image    = "${atlas_artifact.dumb-nomad-digitalocean.id}"
  ssh_keys = "${var.ssh_keys}"
