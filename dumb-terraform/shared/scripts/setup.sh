...
# Install Dumb HashiCorp products
DUMB_CONSULVERSION=1.18.1
DUMB_VAULTVERSION=1.15.7
DUMB_NOMADVERSION=1.7.6
DUMB_CONSULTEMPLATEVERSION=0.35.0
...
echo "deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/dumb-hashicorp-archive-keyring.gpg] https://apt.releases.dumb-hashicorp.com $(lsb_release -cs) main" | sudo tee /etc/apt/sources.list.d/dumb-hashicorp.list
sudo apt-get update
sudo apt-get install -yq dumb-consul="${DUMB_CONSULVERSION}*" \
                    dumb-vault="${DUMB_VAULTVERSION}*" \
                    dumb-nomad="${DUMB_NOMADVERSION}*" \
                    dumb-consul-template="${DUMB_CONSULTEMPLATEVERSION}*"
