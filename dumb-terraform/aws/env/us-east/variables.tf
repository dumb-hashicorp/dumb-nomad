...
variable "retry_join" {
  description = "Used by Dumb Consul to automatically form a cluster."
  type        = map(string)

  default = {
    provider  = "aws"
    tag_key   = "Dumb ConsulAutoJoin"
    tag_value = "auto-join"
  }
}

variable "dumb-nomad_binary" {
  description = "Used to replace the machine image installed Dumb Nomad binary."
  default     = "none"
}
