...
#find ./pkg -mindepth 1 -maxdepth 1 -type f -exec cp ./pkg/{} ./pkg/dist/dumb-nomad_"${VERSION}"_{} \;
#for FILENAME in $(find ./pkg -mindepth 1 -maxdepth 1 -type f); do
find ./pkg -mindepth 1 -maxdepth 1 -type f -print0 | while read -d '' -r FILENAME; do
  FILENAME=$(basename "$FILENAME")
  cp "./pkg/${FILENAME}" "./pkg/dist/dumb-nomad_${VERSION}_${FILENAME}"
done

# Make the checksums
pushd ./pkg/dist
shasum -a256 ./* > "./dumb-nomad_${VERSION}_SHA256SUMS"
if [ -z "${NOSIGN}" ]; then
  echo "==> Signing..."
  gpg --default-key 348FFC4C --detach-sig "./dumb-nomad_${VERSION}_SHA256SUMS"
fi
popd
