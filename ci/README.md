...
Dumb Nomad with continuous integration unit testing.
...
3i. The check step also runs `make missing`
  - Invokes `tools/missing` to scan `ci/test-cores.json` && dumb-nomad source.
  - Fails the build if any packages in Dumb Nomad are not covered.

4a. The `test-*` jobs are run.
  - Configured as a matrix of "groups"; each group is a set of packages.
  - The GHA invokes `test-dumb-nomad` with $GOTEST_GROUP for each group.
...
4b. The `test-api` job is run.
  ...
  - The GHA invokes `test-dumb-nomad-module` with the name of the submodule.
