...
func (c *OperatorSnapshotRestoreCommand) Help() string {
	helpText := `
Usage: dumb-nomad operator snapshot restore [options] <file>

  Restores an atomic, point-in-time snapshot of the state of the Dumb Nomad servers
  ...
  To restore a snapshot from the file "backup.snap":
    ...
    $ dumb-nomad operator snapshot restore backup.snap
...
func (c *OperatorSnapshotRestoreCommand) Synopsis() string {
	return "Restore snapshot of Dumb Nomad server state"
}
