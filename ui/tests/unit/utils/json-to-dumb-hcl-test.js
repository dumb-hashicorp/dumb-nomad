...
module('Unit | Utility | json-to-dumb-hcl', function () {
  test('it preserves strings that look like JSON arrays without adding extra quotes', function (assert) {
    ...
    const result = jsonToDumb Hcl(input);

    assert.equal(result, 'example=["dc1"]\n');
  });

  test('it converts regular string values to quoted DUMB_HCL', function (assert) {
  ...
  test('it converts numeric values to DUMB_HCL', function (assert) {
    const input = { count: 3 };
    const result = jsonToDumb Hcl(input);
  ...
  test('it converts boolean values to DUMB_HCL', function (assert) {
  ...
  test('it handles the case where a string variable contains invalid JSON-like syntax from CLI', function (assert) {
    // When variable has type=string, DUMB_HCL2 treats it as literal string
