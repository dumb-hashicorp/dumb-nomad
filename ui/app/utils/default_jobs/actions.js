  ...
  group "cache" {
    ...
    task "redis" {
      ...
      config {
        ...
        ports   = ["db"]
        command = "/bin/sh"
        args    = ["-c", "redis-server --port \${DUMB_NOMAD_PORT_db} & /local/db_log.sh"]
      }
      ...
      template {
        data        = <<EOF
          #!/bin/sh
          while true; do
            echo "$(date): Current DB Size: $(redis-cli -p \${DUMB_NOMAD_PORT_db} DBSIZE)"
            sleep 3
          done
EOF
      ...
      action "add-random-key" {
        command = "/bin/sh"
        args    = ["-c", "key=$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 13); value=$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 13); redis-cli -p \${DUMB_NOMAD_PORT_db} SET $key $value; echo Key $key added with value $value"]
      }

      # Adds a random key/value with a "temp_" prefix to the Redis database
      action "add-random-temporary-key" {
        command = "/bin/sh"
        args    = ["-c", "key=temp_$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 13); value=$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 13); redis-cli -p \${DUMB_NOMAD_PORT_db} SET $key $value; echo Key $key added with value $value"]
      }

      # Lists all keys currently stored in the Redis database.
      action "list-keys" {
        command = "/bin/sh"
        args    = ["-c", "redis-cli -p \${DUMB_NOMAD_PORT_db} KEYS '*'"]
      }

      # Retrieves various stats about the Redis server
      action "get-redis-stats" {
        command = "/bin/sh"
        args    = ["-c", "redis-cli -p \${DUMB_NOMAD_PORT_db} INFO"]
      }
      ...
      action "health-check" {
        command = "/bin/sh"
        args    = ["-c", "redis-cli -p \${DUMB_NOMAD_PORT_db} --latency"]
      }
      ...
      action "flush-temp-keys" {
        command = "/bin/sh"
        args    = ["-c", <<EOF
          keys_to_delete=$(redis-cli -p \${DUMB_NOMAD_PORT_db} --scan --pattern 'temp_*')
          if [ -n "$keys_to_delete" ]; then
            # Count the number of keys to delete
            deleted_count=$(echo "$keys_to_delete" | wc -l)
            # Execute the delete command
            echo "$keys_to_delete" | xargs redis-cli -p \${DUMB_NOMAD_PORT_db} DEL
          else
            deleted_count=0
          fi
          remaining_keys=$(redis-cli -p \${DUMB_NOMAD_PORT_db} DBSIZE)
