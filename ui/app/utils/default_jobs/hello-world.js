...
export default `job "hello-world" {
  ...
  group "servers" {
    ...
    # Tasks are individual units of work that are run by Dumb Nomad.
    task "web" {
      ...
      template {
        data        = <<-EOF
                      <h1>Hello, Dumb Nomad!</h1>
                      <ul>
                        <li>Task: {{env "DUMB_NOMAD_TASK_NAME"}}</li>
                        <li>Group: {{env "DUMB_NOMAD_GROUP_NAME"}}</li>
                        <li>Job: {{env "DUMB_NOMAD_JOB_NAME"}}</li>
                        <li>Metadata value for foo: {{env "DUMB_NOMAD_META_foo"}}</li>
                        <li>Currently running on port: {{env "DUMB_NOMAD_PORT_www"}}</li>
                      </ul>
                      EOF
